...
| [Gitleaks](https://github.com/gitleaks/gitleaks) | NHI2 | Static secrets: API keys, access tokens, PATs, private keys, passwords (default ruleset) | Git repos incl. full history (`git log -p`), directories/files, stdin; pre-commit hook; GitHub Action | Local filesystem read only; no platform credentials or API access required; read-only (writes only optional local report file) | Hardcoded/committed secrets with rule ID, file, line, commit, author, unique fingerprint; json/csv/junit/sarif reports; baseline diffing for new-findings-only | Regex/entropy based: unclassified secret formats missed, high-entropy false positives; no liveness verification, so revoked and active secrets look identical; only scans content it is pointed at | Findings written to stdout or a local report file (`--report-path`); no network functionality documented; docs contain no explicit telemetry statement | MIT; feature-complete per maintainer (security patches only, focus moved to Betterleaks); latest v8.30.1 Mar 2026 |
